...
-- sal/전체sal 합계
select   sal ,  sum(sal ) over()  ,  sal /  sum(sal ) over()  
from emp;
...
--각 회원의 포인트가 전체대비 몇 % 차지하는지 비율 구하기
select  name, point 
from acorntbl;
...
--point 기준으로 누적값 구하기 ( 처음부터 현재행까지의 합)
select  point,   sum(point)  over( 
order by point
rows  between  unbounded  preceding   and  current row 
)
...
--unbounded  preceding   제한이없는 이전 (처음부터 )
...
--decode  함수   페이지 185 (달력만들기 )     p.189 emp사용하여  부서별 직급별 사원수 구하기 
...
select  deptno, job, decode(job ,'MANAGER',9)
from emp
order by 1;
...
select deptno, count(  decode(job ,'MANAGER', 8))
from emp  
group by deptno
order by 1;
...
-- 세일즈맨 직급 인원 수 구하기 
-- 전체인원수 
-- 매니저 직급의 인원수를 부서벼로 구하기 

--그룹바이된 결과를 조회할때는  그룹바이 되기 전 상태를 잘 만들자 !!!
--눈으로 그룹바이 시도후 그룹바이 진행


--pivot사용하여 부서별 직급별 인원 수 구하기

select  * 
from (  select deptno, job, empno from  emp)

 pivot(   count(empno)   for  job    in( 'CLERK' as  "CLERK" ,'MANAGER' as  "MANAGE") )  --for job  열로 나올 항목
;
...
--unpivot
--pivot행, 열의 집계 데이터를 다시 원래상태로 돌리는 것을 말한다
...
-- 실습하기
-- 210 연습문제  (decode 사용하기)  풀기
-- 1(두식님),  2(성빈님) ,  3 (성원님)   4  (지원님)
-- 5 (성욱님),  7 (승재님) ,  8 (희연님) , 9 (현희님)


--join 연습  (inner 조인 연습하기)
--hy대학교 수강신청 프로그램 pdf (조회) 
 

 --https://special-rainstorm-825.notion.site/2-2f73c1e6781e493094ec53488e439955?pvs=74
 -- 데이터베이스 노션 sqld 시험 
 -- 영진sqld 시험
...
from student;
 ...
 select count(*) from student;
  ...
  --3월생 수 
  select 
    count(decode(to_char( birthday ,'mm') ,'01',8))  || 'EA',
...
FROM student;
  ...
  --국번만 가져오기 
  --)괄호위치를 찾아서  -1하면  가져올 개수를  지역에 구할 수 있다 
    ...
    select   tel,    instr(tel, ')')  , instr(tel, ')')-1  from student;
     ...
     decode( substr(tel, 1, instr(tel, ')')-1 )  ,'02','s')
     from student;
  
  
     --서울 인원 수 구하기
     select      
     count(decode( substr(tel, 1, instr(tel, ')')-1 )  ,'02','s'))
     from student;
...
COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 02, 0)) || 'EA(' || ROUND(COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 02, 0)) * 100 / COUNT(*),0) || '%)'
AS SEOUL,
COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 031, 0)) || 'EA(' || ROUND(COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 031, 0)) * 100 / COUNT(*),0) || '%)' 
AS GYEONGGI,
COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 051, 0)) || 'EA(' || ROUND(COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 051, 0)) * 100 / COUNT(*),0) || '%)'
AS BUSAN,
COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 052, 0)) || 'EA(' || ROUND(COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 052, 0)) * 100 / COUNT(*),0) || '%)'
AS ULSAN,
COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 053, 0)) || 'EA(' || ROUND(COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 053, 0)) * 100 / COUNT(*),0) || '%)'
AS DAEGU,
COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 055, 0)) || 'EA(' || ROUND(COUNT(DECODE(SUBSTR(TEL, 1, INSTR(TEL,')')-1), 055, 0)) * 100 / COUNT(*),0) || '%)'
AS GYEONGNAM
FROM STUDENT;
...
-- 9번 현희님  ratio_to_report(컬럼명) over()      --컬럼명의 전체합계에 대해서 비율을 구함
...
sal,
sum(sal) over(),
--sal/ sum(sal) over()*100     아래와 같이  ratio_to_report() 사용 가능 
